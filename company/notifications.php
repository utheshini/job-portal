...
function getNotificationCounts($conn, $userId) {
    $response = ['status_changes' => 0, 'new_mails' => 0, 'new_replies' => 0];

    // Count unread status changes
    $sqlStatusChanges = "SELECT COUNT(*) AS status_changes FROM apply_job_post WHERE id_company = ? AND status IN ('pending')";
    if ($stmt = $conn->prepare($sqlStatusChanges)) {
        ...
        if ($resultStatusChanges) {
            ...
            echo "Error executing status changes query: " . $stmt->error;
        }
        $stmt->close();
    } else {
        echo "Error preparing status changes query: " . $conn->error;
    }

    // Count new unread mails
    $sqlNewMails = "SELECT COUNT(*) AS new_mails FROM mailbox WHERE id_touser = ? AND createdAt >= DATE_SUB(NOW(), INTERVAL 1 DAY) AND fromuser = 'user'";
    if ($stmtMails = $conn->prepare($sqlNewMails)) {
        ...
        if ($resultMails) {
            ...
            echo "Error executing new mails query: " . $stmtMails->error;
        }
        $stmtMails->close();
    } else {
        echo "Error preparing new mails query: " . $conn->error;
    }

    // Count new unread replies
    $sqlNewReplies = "SELECT COUNT(*) AS new_replies FROM reply_mailbox WHERE id_touser = ? AND usertype = 'user' AND createdAt >= DATE_SUB(NOW(), INTERVAL 1 DAY)";
    if ($stmtReplies = $conn->prepare($sqlNewReplies)) {
        ...
        if ($resultReplies) {
            ...
            echo "Error executing new replies query: " . $stmtReplies->error;
        }
        $stmtReplies->close();
    } else {
        echo "Error preparing new replies query: " . $conn->error;
    }
...
// echo json_encode(getNotificationCounts($conn, $userId));
?>
    ...
    <ul class="dropdown-menu">
        ...
        <li>
            <ul class="menu">
                ...
                <li><a href="#"><i class="fa fa-warning text-yellow"></i> <?= $notificationCounts['status_changes'] ?> applications received</a></li>
                <?php endif; ?>
                ...
                <li><a href="#"><i class="fa fa-reply text-blue"></i> <?= $notificationCounts['new_replies'] ?> new replies received</a></li>
                <?php endif; ?>
            </ul>
        </li>
    </ul>
    ...
    function updateNotifications() {
        $.ajax({
            ...
            success: function(data) {
                var totalNotifications = parseInt(data.status_changes) + parseInt(data.new_mails) + parseInt(data.new_replies);
                ...
                if (data.new_replies > 0) {
                    menuHtml += '<li><a href="#"><i class="fa fa-reply text-blue"></i> ' + data.new_replies + ' new replies received</a></li>';
                }
                if (menuHtml === '') {
