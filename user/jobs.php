...
// Function to check if a job has been saved by the logged-in user
function isJobSaved($conn, $userId, $jobId) {
  $sql = "SELECT * FROM saved_jobs WHERE user_id = ? AND job_id = ?";
...
if (!empty($search)) {
    $whereClauses[] = "(jp.jobtitle LIKE '%$search%' OR jp.description LIKE '%$search%' OR c.companyname LIKE '%$search%')";
}
        ...
        <div class="container">
          ...
          <?php if ($result->num_rows > 0): ?>
            <?php while ($row = $result->fetch_assoc()): ?>
              ...
              $jobSaved = isset($_SESSION['id_user']) ? isJobSaved($conn, $_SESSION['id_user'], $row['id_jobpost']) : false;
              ?>
         <div class="attachment-block clearfix">
            <img class="attachment-img" src="../uploads/logo/<?php echo htmlspecialchars($row['logo']); ?>" alt="Attachment Image">
            <div class="attachment-pushed">
                <h4 class="attachment-heading">
                    <a href="view-job-post.php?id=<?php echo htmlspecialchars($row['id_jobpost']); ?>"><?php echo htmlspecialchars($row['jobtitle']); ?></a>
                ...
                <div class="attachment-text">
                    ...
                    <?php if (!$jobSaved && !empty($_SESSION['id_user'])): ?>
                        <form action="save-job.php" method="POST">
                            <input type="hidden" name="job_id" value="<?php echo htmlspecialchars($row['id_jobpost']); ?>">
                    ...
                    <?php else: ?>
                        <button type="button" disabled class="btn btn-secondary">Saved</button>
                    <?php endif; ?>
                </div>
            </div>
        </div>
